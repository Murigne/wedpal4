      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="bg-white/20 backdrop-blur-sm rounded-lg p-4 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pt-2 pb-4">
            {filteredVendors.map(vendor => <div key={vendor.id} className="bg-white rounded-lg shadow-md overflow-hidden group relative">
                <div className="absolute top-2 right-2 z-10">
                  <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full bg-white/80 hover:bg-white shadow-sm" onClick={() => handleToggleFavorite(vendor.id)}>
                    <Heart className={`h-4 w-4 ${favoriteVendors.includes(vendor.id) ? 'fill-pink-500 text-pink-500' : 'text-muted-foreground'}`} />
                  </Button>
                </div>
                ...
                <div className="p-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <Button className="text-sm px-3 py-1 h-auto bg-pink-500 hover:bg-pink-600 text-white" onClick={() => handleViewDetails(vendor.id)}>
                      View Details
                    </Button>
                  </div>
                </div>
